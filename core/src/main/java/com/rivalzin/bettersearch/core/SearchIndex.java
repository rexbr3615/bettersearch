...
public final class SearchIndex<T> {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            if (query.tokens.length == 0) {
                ...
                for (SearchField field : entry.fields) {
                    ...
                    MatchPolicy fieldPolicy = switch (field.source) {
                        case SearchField.SOURCE_NATIVE, SearchField.SOURCE_ENGLISH -> policy;
                        case SearchField.SOURCE_FOREIGN -> foreignPolicy;
                        default -> strict; // id e tooltip nunca aceitam erro de digitacao
                    };
    ...
    private static int sourceBonus(byte source) {
        return switch (source) {
            case SearchField.SOURCE_NATIVE -> BONUS_NATIVE;
            case SearchField.SOURCE_ENGLISH -> BONUS_ENGLISH;
            case SearchField.SOURCE_FOREIGN -> BONUS_FOREIGN;
            case SearchField.SOURCE_ID -> BONUS_ID;
            default -> BONUS_TOOLTIP;
        };
    }
}
