...
public final class BetterSearchConfigScreen extends OptionRowsScreen {
    ...
    protected void buildTabs() {
        ...
        for (int i = 0; i < tabs.length; i++) {
            ...
            addFixed(new TabButton(x, MARGIN, width,
                    ComponentCompat.translatable("bettersearch.config.tab." + value.key),
                    () -> tab == value,
                    () -> select(value)));
        }
    }
        ...
        switch (tab) {
            ...
            case MATCHING: {
                ...
                addSlider("min_typo_length", 3, 10, 1, () -> settings.minTypoLength,
                        ...
                        value -> ComponentCompat.translatable("bettersearch.config.value.letters", value));
                ...
                addSlider("fuzzy_threshold", 0, EFFORT_STEPS, 1,
                        ...
                        v -> settings.fuzzyThreshold = FUZZY_LEVELS[MathHelper.clamp(v, 0, EFFORT_STEPS)],
                ...
                addSlider("cross_field_threshold", 0, EFFORT_STEPS, 1,
                        ...
                        v -> settings.crossFieldThreshold = CROSS_FIELD_LEVELS[MathHelper.clamp(v, 0, EFFORT_STEPS)],
    ...
    private String enabledLanguagesLabel() {
        int total = LanguageCatalog.available().size();
        int enabled = settings.indexesAllLanguages() ? total : settings.languages.size();
        return ComponentCompat.translatable("bettersearch.config.value.ratio", enabled, total);
    }
    ...
    protected String panelDefaultTitle() {
        return ComponentCompat.translatable("bettersearch.config.title");
    }
    ...
    private static String typoToleranceLabel(int value) {
        switch (value) {
            case 0:  return ComponentCompat.translatable("bettersearch.config.value.off");
            case 1:  return ComponentCompat.translatable("bettersearch.config.value.low");
            case 2:  return ComponentCompat.translatable("bettersearch.config.value.normal");
            default: return ComponentCompat.translatable("bettersearch.config.value.high");
        }
    }
    ...
    private static String resultCountLabel(int value) {
        return value == 0
                ? ComponentCompat.translatable("bettersearch.config.value.unlimited")
    ...
    private static String suggestionLimitLabel(int value) {
        return value >= SUGGESTION_LIMIT_MAX
                ? ComponentCompat.translatable("bettersearch.config.value.too_many")
                : ComponentCompat.literal(Integer.toString(value));
    }
}
