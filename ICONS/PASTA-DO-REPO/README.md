...
Minecraft **26.2**, **26.1–26.1.2**, **1.21.11**, **1.21.9–1.21.10**, **1.21.4**, **1.21.1**,
**1.20.1**, **1.19.2**, **1.18.2**, **1.16.5** e **1.12.2**, no NeoForge, no Fabric e no
Forge — só cliente, não precisa estar no servidor.
...
Com o **JEI** instalado, a busca dele também passa a perdoar erro de digitação e a achar item
pelo nome em outro idioma. O JEI busca primeiro; o Better Search só entra quando ele não achou
nada, e os prefixos próprios do JEI (`#tag`, `$cor`, `^id`, `%aba`) continuam sendo dele. Dá
para desligar em **Avançado → Busca no JEI**.

Na **1.20.1** e na **1.21.1** o mesmo vale para o **EMI**. Nas versões mais novas o EMI ainda
não foi portado, então a opção nem aparece na tela delas.
...
Depois de `/invite`, `/group`, `/grupo`, `/party` ou `/party invite`, o mod passa a oferecer os
jogadores online com a mesma tolerância a erro do resto da busca. É para o caso comum em
servidor: o comando de party é do plugin, o Brigadier não sabe que aquele argumento é um
jogador, e o autocompletar do jogo não oferece nada. Aqui `stev`, `stve` ou `Steev` sugerem
*Steve* do mesmo jeito.

Liga e desliga junto com **Sugestão de Jogadores**, na aba Avançado.
...
| 1.21.4 | `bettersearch-neoforge-1.21.4-*.jar` | NeoForge 21.4.x (a linha inteira, das betas ao 21.4.157+) |
| 1.21.4 | `bettersearch-fabric-1.21.4-*.jar` | Fabric + Fabric API |
...
| 1.19.2 | `bettersearch-forge-1.19.2-*.jar` | Forge 43.x |
| 1.19.2 | `bettersearch-fabric-1.19.2-*.jar` | Fabric + Fabric API |
| 1.18.2 | `bettersearch-forge-1.18.2-*.jar` | Forge 40.x |
| 1.18.2 | `bettersearch-fabric-1.18.2-*.jar` | Fabric + Fabric API |
| 1.16.5 | `bettersearch-forge-1.16.5-*.jar` | Forge 36.x |
| 1.16.5 | `bettersearch-fabric-1.16.5-*.jar` | Fabric + Fabric API |
| 1.12.2 | `bettersearch-forge-1.12.2-*.jar` | Forge 14.23.5.28xx |

Na 1.21.4 o **JEI só existe para NeoForge** (decisão do próprio JEI) e o **EMI não existe**
para nenhum loader; no Fabric dessa versão o visualizador é o **REI**.

Na 1.12.2 o visualizador integrado é o **JEI** — o que inclui o **NEI** do covers1624, que
roda em cima do JEI. REI e EMI não existem para essa versão, então são as duas únicas opções
que não aparecem no menu dela.
...
de cada versão — 8 para 1.12.2 e 1.16.5, 17 para 1.18.2–1.20.1, 21 para a 1.21.x, 25 para a
26.x — o Gradle baixa sozinho.
...
| Sugestão de Jogadores | ligado | `/msg Steev` e `/party invite stev` sugerem *Steve* |
...
As telas de opção do JEI, do EMI e do REI usam o ícone de cada um desses mods, tirado do
próprio jar, só para identificar de qual mod a opção fala. Os logos são dos seus autores:

| Mod | Autor | Licença do mod |
|---|---|---|
| Just Enough Items (JEI) | mezz | MIT |
| EMI | emi (Emily) | MIT / CC0 (assets) |
| Roughly Enough Items (REI) | shedaniel | MIT |

O Better Search não redistribui nenhum desses mods — só o ícone, como identificação visual
da opção correspondente. Se algum autor preferir que o ícone dele não seja usado, é só abrir
uma issue que eu troco por um desenho genérico.
